...
export function PermissionsPage({ permissions = [] }) {
    ...
    // Reset to first page when filters or underlying permissions change
    React.useEffect(() => {
        setCurrentPage(1);
    }, [search, permissionsState.length]);
    ...
    const startItem = filteredPermissions.length === 0 ? 0 : startIndex + 1;
    const endItem = Math.min(startIndex + paginatedPermissions.length, filteredPermissions.length);
    ...
    const handleSubmitAdd = async (e) => {
        ...
        const guard = DEFAULT_GUARD_NAME;
                ...
                {filteredPermissions.length > ITEMS_PER_PAGE && (
                    <div className="flex items-center justify-between px-5 py-3 border-t border-slate-200 bg-slate-50">
                        <div className="text-xs text-slate-600">
                            ...
                            <span className="font-semibold">{filteredPermissions.length}</span> permissions
                        </div>
